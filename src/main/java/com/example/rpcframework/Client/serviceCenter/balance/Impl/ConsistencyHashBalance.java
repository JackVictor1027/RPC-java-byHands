...
public class ConsistencyHashBalance implements LoadBalance {
    ...
    //static:既可以以静态的方式调用来作为工具；也可以以以实例化的方式来使用
    private static String getServer(String node,List<String> serviceList){
        init(serviceList);
    ...
    public void addNode(String node) {
        if(!realNodes.contains(node)){
            realNodes.add(node);
            System.out.println("真实节点添加["+node+"]上线添加");
            for (int i = 0; i < VIRTUAL_NUM; i++) {
    ...
    public void delNode(String node) {
        if(realNodes.contains(node)){
            ...
            for (int i = 0; i < VIRTUAL_NUM; i++) {
                ...
                int hash = getHash(virtualNode);
                shards.remove(hash);
                System.out.println("虚拟节点["+virtualNode+"]hash:"+hash+",下线");
            }
        }
    }
    ...
    public String balance(List<String> addressList) {
        ...
        return getServer(randomNode,addressList);
    }
}
